...
export const useRandom = () => {
    const [faker, setFaker] = useState(fak);
    const nodes = axios.get(urlValue + "getconnectionnames");
    const response = axios.get(urlValue + "getallconnections")
    const graphInput = response.data;
    const nodesInput = nodes.data;
    ...
    const randomGraph = useCallback(() => {
        // Create the graph
        const graph = erdosRenyi(UndirectedGraph, { order: 100, probability: 0.5 });
        console.log(graph)
